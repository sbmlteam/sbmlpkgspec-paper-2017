...
%DIF DEL submissions/revision-1/sbmlpkgspec-paper.tex   Fri Jun 23 17:02:17 2017
%DIF ADD sbmlpkgspec-paper.tex                          Fri Aug  4 17:08:46 2017
%%                                      %
...
\parttitle{Results} Specification documents for SBML Level~3 extensions (known as \emph{packages} in SBML) are made more uniform with the use of a standard template.  \sbmlpkg{} is a LaTeX class that provides a \DIFdelbegin \DIFdel{common document format }\DIFdelend \DIFaddbegin \DIFadd{document framework }\DIFaddend for SBML Level~3 package specifications, to simplify the work of document authors while improving the overall quality of the family of SBML specifications.
\end{abstract}
...
SBML (the Systems Biology Markup Language) is an XML-based (Extensible Markup Language) format that has become a community standard for the storage, communication and interchange of models in systems biology~\cite{hucka2003the, waltemath2014meeting}.  The format has evolved in a community-driven fashion, with contributions from dozens of people worldwide over more than a decade and a half.  The latest generation of SBML is SBML Level~3, which is structured as a self-sufficient core and optional \emph{SBML Level~3 packages} that can be used to extend the core's syntax and semantics~\cite{hucka2015the}.  The definition of each SBML Level~3 package is written in a formal specification document that is made freely available\DIFdelbegin \DIFdel{to everyone}\DIFdelend ~\cite{schreiber2015specifications, schreiber2016specifications}.
\DIFaddbegin 

\DIFaddend SBML packages are \DIFdelbegin \DIFdel{introduced }\DIFdelend \DIFaddbegin \DIFadd{produced }\DIFaddend and ratified according to an explicit SBML Development Process (\url{http://sbml.org/Documents/SBML_Development_Process})\DIFdelbegin \DIFdel{; this }\DIFdelend \DIFaddbegin \DIFadd{.  This }\DIFaddend process also defines \DIFdelbegin \DIFdel{a template for }\DIFdelend the content that each specification document should contain.  \DIFaddbegin \DIFadd{However, it is challenging for prospective specification creators to generate a clear and complete specification document from scratch.  The availability of an existing framework provides a starting point and helps authors in several respects:
}\DIFaddend 

\DIFaddbegin \begin{itemize}

\item \DIFadd{It helps ensure that required information is not forgotten from the document
}

\item \DIFadd{It saves writing time by providing well-tested commands for formatting elements that are commonly used in SBML specification documents
}

\item \DIFadd{It provides a cohesive look and feel for all SBML specifications
}

\end{itemize}

\DIFadd{For these reasons, \sbmlpkg{} was developed to provide a document framework for SBML package specification documents.
}


\DIFaddend \section*{Main Text}
